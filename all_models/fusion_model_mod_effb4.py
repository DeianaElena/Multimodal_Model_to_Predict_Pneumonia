...
class Fusion_Model_mod_eff4(pl.LightningModule):
    def __init__(self, image_input_dim, text_input_dim, learning_rate):
        super(Fusion_Model_mod_eff4, self).__init__()
        self.conv_block = nn.Sequential(
            # nn.Conv2d(image_input_dim, 512, kernel_size=(1, 1), stride=(1, 1), bias=False),
            #0.05 % more in sensitivity with the following
            nn.Conv2d(image_input_dim, 1792, kernel_size=(1, 1), stride=(1, 1), bias=False),
        ...
        self.fc = nn.Sequential(
            nn.Linear(1792+text_input_dim, 256),  # if both previous models output 2 classes
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(256, 2),  
            nn.LogSoftmax(dim=1)
